  ...
  return (
    <div className={`${maximized ? 'space-y-0.5' : 'space-y-2'} w-full`}>
      {rows.map((rowFiles, rowIndex) => {
        ...
        const gap = maximized ? 2 : 8
        const totalGap = (rowFiles.length - 1) * gap
        const availableWidth = containerWidth - (maximized ? 4 : 48) - totalGap

        return (
          <div key={rowIndex} className={`flex ${maximized ? 'gap-0.5' : 'gap-2'}`}>
            {rowFiles.map((file) => {
              const aspectRatio = file.width && file.height ? file.width / file.height : 1
                  ...
                  >
                    ...
                    {getThumbnailUrl(file) ? (
                      <NextImage
                        ...
                        placeholder="blur"
                        blurDataURL={getShimmerPlaceholder(Math.round(itemWidth), rowHeight)}
                        loading="lazy"
                        quality={85}
                      />
                    ) : file.type === 'AUDIO' ? (
