/**
 * KieKlingV25TurboTextToVideoProAdapter - Kie.ai Kling v2.5 Turbo Text to Video Pro
 *
 * 对应模型: kie-kling-v2-5-turbo-text-to-video-pro
 * 功能: 文生视频（v2.5 Turbo Pro，快速生成，专业质量，支持宽高比）
 *
...
export class KieKlingV25TurboTextToVideoProAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      this.log('info', 'Creating Kie Kling v2.5 Turbo Text to Video Pro task', payload)
      ...
      this.log('info', `Kling v2.5 Turbo Text to Video Pro task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie Kling v2.5 Turbo Text to Video Pro dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      ...
      this.log('error', 'Failed to check Kling v2.5 Turbo Text to Video Pro task status', error)
