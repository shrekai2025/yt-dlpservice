/**
 * KieVeo3Adapter - Kie.ai Veo 3 Video Generation
 *
 * 对应模型: kie-veo3 和 kie-veo3-fast
 * 功能: 文生视频和图生视频（Google Veo 3模型）
 *
...
interface KieVeo3StatusResponse {
  code: number
  msg: string
  data: {
    taskId: string
...
export class KieVeo3Adapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      if (request.parameters?.model) {
        ...
        // 根据 slug 自动选择模型
        payload.model = this.config.slug === 'kie-veo3-fast' ? 'veo3_fast' : 'veo3'
      }
      ...
      this.log('info', 'Creating Kie Veo 3 task', payload)

      // 创建任务
      const response = await this.httpClient.post<KieVeo3TaskResponse>(
        '/api/v1/veo/generate',
        payload,
        {
      ...
      this.log('info', `Veo 3 task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie Veo 3 dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      const response = await this.httpClient.get<KieVeo3StatusResponse>(
      ...
      // 成功 (successFlag = 1)
      if (successFlag === 1 && taskResponse?.resultUrls?.length > 0) {
      ...
      this.log('error', 'Failed to check Veo 3 task status', error)
