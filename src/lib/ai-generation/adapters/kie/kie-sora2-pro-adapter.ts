/**
 * KieSora2ProAdapter - Kie.ai Sora 2 Pro Text To Video Generation
 *
 * 对应模型: kie-sora2-pro
 * 功能: 文生视频（基于Sora 2 Pro模型，支持更多参数）
 *
...
interface KieSora2ProTaskResponse {
  code: number
  msg: string
  data: {
    taskId: string
...
interface KieSora2ProStatusResponse {
  code: number
  msg: string
  data: {
    taskId: string
...
interface KieSora2ProResultJson {
  resultUrls: string[]
}

export class KieSora2ProAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      this.log('info', 'Creating Kie Sora 2 Pro task', payload)

      // 创建任务
      const response = await this.httpClient.post<KieSora2ProTaskResponse>(
      ...
      this.log('info', `Sora 2 Pro task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie Sora 2 Pro dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      const response = await this.httpClient.get<KieSora2ProStatusResponse>(
      ...
      if (state === 'success' && resultJson) {
        let parsedResult: KieSora2ProResultJson
        try {
          parsedResult = JSON.parse(resultJson) as KieSora2ProResultJson
        } catch {
          return {
            status: 'ERROR',
      ...
      this.log('error', 'Failed to check Sora 2 Pro task status', error)
