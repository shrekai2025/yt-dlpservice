/**
 * KieNanoBananaEditAdapter - Kie.ai Nano Banana Edit Image Generation
 *
 * 对应模型: kie-nano-banana-edit
 * 功能: 基于 Google Nano Banana Edit 的图像编辑
 */
...
interface KieNanoBananaEditTaskResponse {
  code: number
  msg: string
  data?: {
    taskId: string
...
interface KieNanoBananaEditStatusResponse {
  code: number
  msg: string
  data?: {
    taskId: string
...
export class KieNanoBananaEditAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      if (!request.inputImages || request.inputImages.length === 0) {
        return {
          status: 'ERROR',
          message: 'Nano Banana Edit requires at least one input image',
          error: {
      ...
      const input: Record<string, unknown> = {
        ...
        image_urls: request.inputImages.slice(0, 10), // 最多10张图片
      }
      ...
      this.log('info', 'Creating Kie Nano Banana Edit task', payload)

      // 创建任务
      const response = await this.httpClient.post<KieNanoBananaEditTaskResponse>(
      ...
      this.log('error', 'Kie Nano Banana Edit dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      const response = await this.httpClient.get<KieNanoBananaEditStatusResponse>(
