/**
 * KieWan22A14bTextToVideoTurboAdapter - Kie.ai Wan 2.2 A14B Text to Video Turbo
 *
 * 对应模型: kie-wan-2-2-a14b-text-to-video-turbo
 * 功能: 文生视频（Turbo 快速生成，支持多分辨率）
 *
...
export class KieWan22A14bTextToVideoTurboAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      this.log('info', 'Creating Kie Wan 2.2 A14B Text to Video Turbo task', payload)
      ...
      this.log('info', `Wan 2.2 A14B Text to Video Turbo task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie Wan 2.2 A14B Text to Video Turbo dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      ...
      this.log('error', 'Failed to check Wan 2.2 A14B Text to Video Turbo task status', error)
