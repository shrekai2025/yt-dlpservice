/**
 * KieKlingV2MasterImageToVideoAdapter - Kie.ai Kling v2-1 Master Image to Video
 *
 * 对应模型: kie-kling-v2-1-master-image-to-video
 * 功能: 图生视频（支持首帧/尾帧输入）
 *
...
export class KieKlingV2MasterImageToVideoAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      this.log('info', 'Creating Kie Kling v2-1 Master Image to Video task', payload)
      ...
      this.log('info', `Kling v2-1 Master Image to Video task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie Kling v2-1 Master Image to Video dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      ...
      this.log('error', 'Failed to check Kling v2-1 Master Image to Video task status', error)
