/**
 * KieByteDanceV1ProImageToVideoAdapter - Kie.ai ByteDance V1 Pro Image to Video
 *
 * 对应模型: kie-bytedance-v1-pro-image-to-video
 * 功能: 图生视频（ByteDance Seedance V1 Pro）
 *
...
export class KieByteDanceV1ProImageToVideoAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      this.log('info', 'Creating Kie ByteDance V1 Pro Image to Video task', payload)
      ...
      this.log('info', `ByteDance V1 Pro Image to Video task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie ByteDance V1 Pro Image to Video dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      ...
      this.log('error', 'Failed to check ByteDance V1 Pro Image to Video task status', error)
