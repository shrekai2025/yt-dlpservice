/**
 * KieKlingV2ProAdapter - Kie.ai Kling v2-1 Pro Image to Video
 *
 * 对应模型: kie-kling-v2-1-pro
 * 功能: 图生视频（Pro 专业质量版本，支持首帧/尾帧输入）
 *
 * API文档: https://api.kie.ai/api/v1/jobs/createTask
 * 定价: 5秒视频 50 credits ($0.25), 10秒视频 100 credits ($0.5)
 */
...
export class KieKlingV2ProAdapter extends BaseAdapter {
  /**
   * 调度生成请求
   */
  async dispatch(request: GenerationRequest): Promise<AdapterResponse> {
    try {
      ...
      if (!request.inputImages || request.inputImages.length === 0) {
        return {
          status: 'ERROR',
          message: 'At least one input image is required (首帧)',
          error: {
      ...
      // 如果有第二张图片,作为尾帧 (注意: Pro使用tail_image_url而不是end_image_url)
      if (request.inputImages.length >= 2) {
        input.tail_image_url = request.inputImages[1]
      }
      ...
      this.log('info', 'Creating Kie Kling v2-1 Pro task', payload)
      ...
      this.log('info', `Kling v2-1 Pro task created: ${data.taskId}`)
      ...
      this.log('error', 'Kie Kling v2-1 Pro dispatch failed', error)
  ...
  async checkTaskStatus(taskId: string): Promise<AdapterResponse> {
    try {
      ...
      this.log('error', 'Failed to check Kling v2-1 Pro task status', error)
